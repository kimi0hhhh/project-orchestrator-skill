...
> **能力地图**：本角色可用的外部 skill / MCP 工具与使用纪律，见 `.opencode/protocols/capability-map.md` §2 产品经理（工件契约优先于任何外部模板）。
> **推荐 skill（派发时点名）**：见 `capability-map.md` §2｜推荐：pm-skills 的 `create-prd`(只取方法)、`prioritization-frameworks`、`pre-mortem`、`user-stories`、`interview-script`、`user-personas`。
