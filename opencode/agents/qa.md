...
> **能力地图**：本角色可用的外部 skill / MCP 工具与使用纪律，见 `.opencode/protocols/capability-map.md` §6 测试（工件契约优先于任何外部模板）。
...
2. **禁止向其他 agent 派发子任务**（一层编排）：任何角色不得用 task 工具或子进程
...
6. **测试计划前置**：`17-test-plan` 可在 **S2 契约门禁 PASS 后前置起草**——用例设计只依赖
   `09-api-contract` 与 `02-prd`，不依赖实现代码。收到「前置起草」派发时：只写 17 并 finish；
   到 S5 再收「执行」派发，产出 `18-test-report` 与 `04-defects`。前置的 17 照常走 G-QA-01。
7. **定期心跳**：思考/写码超过 10 分钟无 `progress` 时补
   `heartbeat --agent qa --step "长跑中：<在做什么>"`。看门狗无法区分「深思」与「挂死」，
   定期心跳能防误判中断引发的整轮取证与重派。
