...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.startswith("/api/whoami"):
            # 轻量身份探针（v4.4）：cli.py / board.py 在写任何东西之前先问一句
            # 「这个端口上是不是本工作区的看板」。回答只有几十字节，可每次调用前跑。
            return self._send(200, {"workspace": str(WS),
                                    "port": self.server.server_address[1]})

        if p.startswith("/api/state"):
            limit = int(q.get("limit", ["300"])[0])
            snap = store.snapshot(pid=pid, msg_limit=limit)
            # 身份标记（v4.4）：board.py 靠它判断「这个端口上是不是本工作区的看板」。
            # 不靠猜进程命令行——中文工作区路径在不同控制台代码页下会解码错，
            # 导致自己的服务被误判成别人的（实测踩坑）。只增字段，不影响既有消费方。
            snap["workspace"] = str(WS)
            snap["server_port"] = self.server.server_address[1]
