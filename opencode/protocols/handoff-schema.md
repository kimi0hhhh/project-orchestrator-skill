...
| `20-closeout` | **欠账台账**（未交付/部分交付 → 带责任人与触发条件的待办） | 主 Agent | 用户/下一轮 | S7 节 |
| `retro` | 迭代回顾（四问，第 1 问必须带漏网数） | 主 Agent | 下一轮 | S7 节 |

> **`20-closeout` 存在的理由（v4.4）**：框架原先对「没通过」没有出口。实测教训——
> FundLens 终验判 NOK 后 **S7 从未执行**，4 条未交付 + 4 条部分交付全部断在项目边界上，
> 没有任何机制把它们带进下一轮；retro 也没做，于是「门禁拦住了什么、漏了什么」这个
> 唯一能衡量门禁好坏的数从来没被算出来。**项目可以烂尾，欠账不能凭空消失。**
