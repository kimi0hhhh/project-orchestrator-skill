...
| **实现版本** | v4.1 | **v4.4**（含协议级变更，其余实现待移植） | **v4.2**（架构方向修正） | v4.1 |
...
| **角色契约** | 6 份（`agents/*.md`，装到 `~/.zcode/agents/`） | **8 份**（7 角色 + `orchestrator.md` 主契约，装到 `.opencode/agents/`） | 7 份（`00-orchestrator` + `01~06`，随 install 铺进工作区 `.workbuddy/agents/`） | 6 份平台无关契约（`tools/build_universal.py` 从 ZCode 源构建） |
...
| **协议文件形态** | 规则内联在 SKILL.md | `protocols/` 5 文件（含 capability-map）+ **`scoring/` 评分尺（v4.4）** | `protocols/` 3 文件 + role-pool 池 | `universal/protocols/` 3 文件 + platform-config |
...
- **共享（改一处全家族生效）**：铁律、阶段状态机、工件编号、门禁标准、参谋会/分诊/快车道机制、runtime（Python 标准库）；
  > v4.4 起 OpenCode 版另带 `scoring/` 评分尺与 `opencode/runtime/` 增量覆盖层；
  > 其中**门禁标准与工件编号的改动属协议级**，需按下方同步纪律移植到其余实现。
