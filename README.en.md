# Project Orchestrator · A Multi-Agent Orchestration Protocol for Agent Dev Tools

**One-liner**: A collaboration protocol of *one project owner + 6 single-accountability roles + on-demand advisors + stage gates* — the main agent never touches the work itself; it dispatches, reviews, and gates. Single writer, one-layer orchestration, read-only advisors, no gate-keeping shortcuts. This is **not a tool-specific trick**: it is a collaboration pattern designed for any agent dev tool with sub-agent dispatch. **ZCode and OpenCode are the first two implementations** (this repo ships the ZCode edition first). Delivery is gated — no promise of one-shot perfection.
...
Not a prompt pack — an installable **orchestration protocol** (1 skill + 6 role contracts). Once loaded, the main agent behaves like a project owner:
...
- **Two evidence-backed mechanisms** — the **star advisor pattern** (read-only advisors challenge a draft for one round; in the three-condition controlled experiment, PRD blind score 21 → 22.5, end-to-end +13%) and **dynamic staffing L0–L2** (simple tasks get one agent, so the ~15× token cost of multi-agent — measured on research tasks — never burns on work that doesn't need it).

The protocol is platform-decoupled: swap the host tool, swap the adapter — not the protocol. The ZCode edition is shipped; the OpenCode edition reuses the same contracts and stage definitions verbatim.
...
This protocol is an engineering convergence of 2025–2026 multi-agent research. Each row: core finding → what we adopted / where we took a measured position.
...
| [Cognition · Don't Build Multi-Agents](https://cognition.com/blog/dont-build-multi-agents) | Parallel writers exchange **implicit decisions** neither can see; merging = conflict | Adopted: one writer per artifact. Diverged: we don't retreat to a single thread — gates + read-only advisors keep the multi-view (e2e +13%) |
...
| [Equal-budget study](https://arxiv.org/abs/2604.02460) | Without token-matched controls you measure compute, not architecture | Adopted: reproduction guide mandates equal budgets. Caveat: our e2e run was not budget-matched (2.2× cost), so +13% is directional evidence only |
...
One sentence sums up all nine: **multi-agent failures cluster around "writing"** — so this protocol confines parallelism to file-isolated S3, and makes everything else (review, challenge, observation) read-only.
...
![Blind-judged scores](docs/assets/scores.svg)

| Condition | Tokens/run | Score | Verdict |
|---|---|---|---|
| Single-agent baseline | 21k | 21.0 | optimal for simple tasks |
| Skill invocation only | 32k | 17.25 | **negative return** (template crowds out substance) |
| This protocol (advisor + revision) | 122k | **22.5** | gains concentrate on metric rigor & edge cases |

End-to-end mini-product (blind-judged, 40-pt): this protocol **35/40** vs sequential single-role **31/40** (+13%, at 2.2× cost):

![E2E comparison](docs/assets/e2e.svg)

Full data and methods (including the n=2 statistical caveat): [docs/EVIDENCE.md](docs/EVIDENCE.md).

## An honest look

**Strengths (directional evidence from controlled runs — n=2, not budget-matched; consistent in direction, pending larger samples)**

1. **Delivers a whole product autonomously**: given a clear brief plus a few clarifying answers, it lands the full chain — requirements → PRD → contract → runnable code → tests (the end-to-end validation target was a backend-less single-page app);
2. **Higher product quality**: +13% blind-judged vs baseline; advisors caught regulatory-grade red lines at draft time (e.g. a "retention north-star metric" for a backend-less product);
...
2. **Token-hungry**: advisor groups, revision rounds and board polling all burn tokens — estimated at several times a single-agent baseline. **Not yet rigorously quantified; pending an equal-budget controlled run** (the existing 2.2× figure is not budget-matched).

**When not to use it**: changes under a day of work; tasks with a single decision chain that won't decompose; when you want a patch, not an artifact chain; when your token/time budget cannot absorb a revision round. Hit any of these and use a single agent — triage L0 exists precisely to write this rule into the protocol.
