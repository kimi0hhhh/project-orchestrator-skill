...
def resolve_port(arg):
    ...
    ★ 老实现的坑：没给参数时直接信 `.port`，而 `.port` 可能是**上一次遗留的陈旧值**
      （实测残留成 80），于是新实例去绑一个无关端口、又因为 `alive(.port)` 恰好为真
      而误报"已在运行"。规则改为：显式参数优先；否则只有当 `.port` 上的服务**真的在应答**
      才沿用它，否则回默认 8788。
    """
    ...
    if alive(stale):
        return stale
    return 8788          # .port 是陈旧值（或文件不存在）→ 回默认端口，不盲从
...
def main():
    # v3.x 修复：stop 写在第二个参数（如 `daemon.py 8788 stop`）时会被忽略，只认 argv[1]
    ...
    # ★ 只认「本次启动真的绑上的那个端口」：server.py 绑定成功后才会重写 .port，
    #   所以判据是「.port 的 mtime 变了 且 该端口在应答」。老实现轮询 read_port()
    #   而不看 mtime，只要任意一个旧实例在旧端口应答就误报成功（实测：起 8788 却报 80）。
    import time
    for _ in range(40):
        time.sleep(0.25)
        if p.poll() is not None:
            print(f"服务进程已退出（退出码 {p.returncode}），请查看日志：{LOG}")
            return 1
        if _mtime(portfile) != before:
            real = read_port()
            if alive(real):
                print(f"看板已脱离终端启动：http://127.0.0.1:{real}　pid={p.pid}　日志={LOG}")
                return 0
    print(f"启动超时（目标端口 {port}），请查看日志：{LOG}")
    return 1
