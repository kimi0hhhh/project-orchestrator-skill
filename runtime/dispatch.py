...
def build_prompt(a, reads, memory):
    ...
    add("")
    add("【硬约束】")
    add("- 禁止修改 runtime/** 与 .zcode/state/**（看板状态归主 Agent 管）")
...
def cmd_prepare(a):
    ...
    if not a.no_spawn and memory is not None:
        print("注意：Agent 工具回传的 agent_xxx 请让子 agent 用 spawn --zcode-agent 自登记（看门狗按它判存活）")
...
def cmd_close(a):
    ...
    print(f"gate-log 建议行：| {date.today().isoformat()} | {a.agent} | "
          f"{f.name} | 【待判】 | {a.summary or ''} |")
    print("收尾：对照 gate-rules 判 PASS/CONCERN/FAIL 写入 .zcode/state/gate-log.md，"
